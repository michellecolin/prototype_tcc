    //
		...
		{	
			...
			$("#myCanvas").mousedown(function(event){ mouseDownEvent(event.clientX, event.clientY);});
			$("#myCanvas").mousemove(function(event){ mouseMoveEvent(event.clientX, event.clientY);});
			$("#myCanvas").mouseup(function(event){ mouseUpEvent(event.clientX, event.clientY);});

			//add event listeners for touch
			$("#myCanvas").on("touchstart",function(event){ mouseDownEvent(event.touches[0].clientX, event.touches[0].clientY);});
		...
		function mouseUpEvent(x, y)
		{
			...
			if (_isDown)
			{
				...
				if (_points.length >= 10)
				{
					...
					handleGestureActions(result.Name, round(result.Score,2));
				}
		...
		function handleGestureActions(action, score) {
